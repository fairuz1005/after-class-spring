...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public ResponseEntity<ResTemplateDto> changePassword(ReqChangePassword reqChangePassword, String authToken) {
        String token = authToken.substring(7, authToken.length());
        String username = jwtUtil.extractUsername(token);
        ...
        user.setPassword(passwordEncoder.encode(reqChangePassword.getNewPassword()));
        userRepository.save(user);

        return new ResponseEntity<>(new ResTemplateDto(null, "Success"), HttpStatus.OK);
    }
